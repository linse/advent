...
type Cmds = [[String]]
type Mem = [(String, Int)]
type ProgState = (([Int], [Int]), Mem, Cmds)

data Prog = Prog { rcvd :: Int, snt :: Int, mem :: Mem, cmdz :: Cmds }
...
initStates f = (progState 0, progState 1)
 where progState p = (initSnds, initMemP p f, cmds f)

runProgs :: Cmds -> (ProgState, ProgState) -> (ProgState, ProgState)
